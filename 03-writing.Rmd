...
`papaja` provides the functions `apa_beeplot`, `apa_lineplot`, and `apa_barplot` to easily plot data from factorial designs.
They are intended to provide a convenient way to obtain a publication-ready figure with only the most necessary arguments;
while maintaining enough flexibility for a lot of customizing.
The basic arguments to these functions are all the same , so you only need to learn one syntax to generate these different types of plots. Moreover, this comes with the additional advantage that you can easily switch between these types of plots by simply replacing some letters in the function call.
...
while the other argments represent chracter strings identifying the columns that contain the subject identifier `id`,
the dependendent variable `dv`,
and the between- or within-subjects `factors`. Currently, up to four factors are supported.
...
- The name of the dependent variable on the y ayis
- 
