...
`papaja` provides an APA CSL file that enables adding annotations to the bibliography.
...
Any references that has an `annote` field will be preceded by the contents of that field, e.g.  `*`.
...
No entry in the `annote` field of the reference is required.
...
- Chunk labels must not contain underscores (`_`) for this to work.
- Precede by non-breaking spaces, e.g.<br />`Figure\ \@ref(fig:chunk-label)`
- If you want to reference sections (i.e., "in Section 1, we describe in more detail..."), make sure that your sections are actually numbered by setting `numbersections: yes` in the document's yaml header.
...
To get an accurate word count `papaja` uses the Lua word count filter in [`rmdfiltr`](https://cran.r-project.org/web/packages/rmdfiltr/index.html), which yields counts that are [comparable to other methods](https://cran.r-project.org/web/packages/rmdfiltr/vignettes/wordcount.html).
