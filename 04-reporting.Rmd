...
```{r}
# load data: Dataset is obtained via Andy Field.
load("data/cosmetic_surgery.rdata")
# compare post-surgery quality of life with pre-surgery quality of life
t_out <- t.test(x = cosmetic_surgery$Post_QoL, y = cosmetic_surgery$Base_QoL, paired = TRUE)
# let's see what the apa_print() output looks like
apa_print(t_out)
```

As you can see, `apa_print()` returns a list of four elements:
List element `estimate` is the sample estimate of mean difference;
element `statistic` is the result of the NHST with test statistic, *df*s, and *p* value;
element `full_result` is both information combined.
The fourth element, `table`, is a table containing test results; while this is `NULL` for a $t$ test,
...
What is most fun is that you can simply drag the `table` element into `apa_table()` to get a full regression table, see Table \@ref(tab:lm-table).

```{r lm-table, results = "asis"}
apa_table(apa_lm$table, caption = "A full regression table.")
```
...
`car` [@R-car], `ezANOVA` [@R-ez], and `afex` [@R-afex]. The problem is: All ANOVA functions provide
...
anova_out <- afex::aov_ez(data = cosmetic_surgery, dv = "Post_QoL", id = "ID", between = c("Clinic", "Gender"), return = "Anova")
apa_anova <- apa_print(anova_out)
```


```{r anova-table, results = 'asis'}
apa_table(apa_anova$table, caption = "A really beautiful ANOVA table.", note = "Note that the column names contain beautiful mathematical copy: This is because the table returned by apa_print() has variable labels.")
```
