...
First of all, the function `printnum()` can be used to include a numeric value into text,
with the correct rounding and with or without a leading zero.
For example, if you want to include a mean into text, just write
...
The function `printp()` is a shorthand version of `printnum()` with the correct defaults set for printing a $p$ value
...
In R, the results of a $t$ test (regardless of the exact type of $t$ test) are stored in an `htest` object.
