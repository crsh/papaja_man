...
Docker is the most widely used containerization approach.
It is open source and free to use but requires some disk space.
For a concise hands-on introduction see [the ROpenSci tutorial](https://ropenscilabs.github.io/r-docker-tutorial/).
A [more detailed introduction](https://docker-curriculum.com/) is available from the Docker project.
Docker containers a configured using so-called Docker files.
To configure a Docker container that includes everything to start writing your manuscript, including an instance of RStudio that you can access through you Browser you can build use the following Docker file:
...
Place this Docker file in your project repository and run the container using the following Bash script (for MacOS or Linux):
...
Place this file in your project repository and execute it.
...
In new you can work as you usually would.

```{block2, docker-r-packages, type='rmdnote'}
Note that if you work in RStudio, whenever you install an R package interactively (i.e., via the GUI or `install.packages()`) they will be lost once you stop the container.
While this may seem inconvenient, it ensures that your software environment is fully self-contained.
```

To make additional R packages available in your container, add them to the Docker file.
For example,
...
Note that all R packages are installed from the MRAN server that provides each package as it was available from CRAN on a particular date in the past (that is, the last day the specified version of R was the most recent release, see the [rocker version information](https://github.com/rocker-org/rocker-versioned/blob/master/VERSIONS.md)).
...
CodeOcean's Seth Green has kindly prepared a [minimal `papaja` example capsule](https://codeocean.com/capsule/8183533/).
